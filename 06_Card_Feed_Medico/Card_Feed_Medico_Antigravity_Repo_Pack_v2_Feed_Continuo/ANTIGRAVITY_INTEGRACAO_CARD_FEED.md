...
## 11. Novo modo: Feed Contínuo estilo Instagram/X 📲

O módulo agora possui um botão:

```text
📲 Feed contínuo
```

Esse modo altera a visualização para:

- uma imagem/card por vez;
- coluna única centralizada;
- rolagem vertical contínua;
- aparência parecida com Instagram, X/Twitter ou Reels estático;
- comentários e explicações logo abaixo de cada imagem;
- ideal para iPhone/iPad.

O modo antigo de múltiplas colunas continua disponível pelo botão:
...
O `index.html` do módulo usa:

```js
localStorage.setItem("cardFeedViewMode", viewMode)
```

Assim, o navegador lembra a preferência do usuário.

### CSS principal

O modo contínuo é ativado com a classe:

```css
body.feed-continuous
```

Não remover essa classe nem os estilos relacionados.

### Teste obrigatório

No navegador:

1. abrir `06_Card_Feed_Medico/index.html`;
2. clicar em `📲 Feed contínuo`;
3. confirmar que os cards aparecem um abaixo do outro;
4. clicar em `▦ Modo grade`;
5. confirmar retorno ao layout em colunas.
